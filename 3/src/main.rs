...
fn get_symbols(all: &[Artifact]) -> Vec<&Artifact> {
    all
        .iter()
        .filter(|art| matches!(art.content, Content::Symbol(_)))
        .collect()
}

fn get_gears(all: &[Artifact]) -> Vec<&Artifact> {
    all.iter()
...
fn get_parts_adjacent_to<'a>(all: &'a[Artifact], pos: &'a Position) -> Vec<&'a Artifact> {
...
{
    let all: Vec<Artifact> = contents
        .enumerate()
        .flat_map(|(i, line)| parse_row(i.try_into().unwrap(), line))
        .collect();
